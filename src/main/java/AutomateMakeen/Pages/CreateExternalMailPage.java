...
// WebElement for the successful message display
     ...
     */
    @FindBy(id = "btn_add_file")
// Button to add a file
            WebElement addFileWebElement;

    @FindBy(id = "attachDiv_txt_FileName")
// Input field for the file name
    WebElement fileNameWebElement;

    @FindBy(id = "attachDiv_btn_ChooseFile")
// Button to choose the file to attach
    WebElement attachedFileWebElement;

    @FindBy(css = "div[class='popup-footer'] ul li p")
// Confirmation button to add the file
    WebElement confirmAddFileWebElement;

    // Method to add a file attachment by specifying file name and path
    public boolean addFile(String fileName, String path){
        addFileWebElement.click();
        fileNameWebElement.sendKeys(fileName);
        File file = new File(path);
        String absolutePath = file.getAbsolutePath();
        attachedFileWebElement.sendKeys(absolutePath);
        exWait.until(ExpectedConditions.visibilityOf(confirmAddFileWebElement));
        boolean flag = validateFileAdded();
        confirmAddFileWebElement.click();
        return flag;
    }

    @FindBy(id = "attachDiv_file_name_0")
// WebElement representing the first added file in the attachment list
    WebElement file1WebElement;

    // Method to validate if the file was added successfully
    public boolean validateFileAdded(){
        return file1WebElement.isDisplayed();
    }
}
