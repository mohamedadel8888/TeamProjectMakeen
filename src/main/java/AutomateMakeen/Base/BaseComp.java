...
public class BaseComp {
    ...
    public String getValidatorState(WebElement validator) {
        switch (validator.getAttribute("class")) {
            case "fa fa-question-circle redText":
                return "Red Circle";
            case "fa arow-none fa-asterisk":
            case "fa fa-asterisk redText":
                return "Asterisk";
            default:
                return "Error !!";
        }
    ...
    public static String getMiladiDate() { // Get the current date in the Gregorian calendar
        LocalDate miladiDate = LocalDate.now();
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy/MM/dd");
        return formatter.format(miladiDate);
    }
